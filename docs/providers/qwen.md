...
already supply `--json-file` or `--json-fd`. The probe runs the executable
resolved for the launched provider, so an alias that inherits Qwen is probed
through its own binary. It never selects headless stream-JSON output.
